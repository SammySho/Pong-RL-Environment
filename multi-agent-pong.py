...
class PongEnv(object):
    ...
    def update_right_paddle(self, action = 0.0):
        """
        This method takes an action and updates the position of the right paddle according to the action
        """
        self.right_vel = action * 2
    ...
    def step(self, action_1, action_2, basic=False):
        """
        This method handles the physics of the environment at each time step, it is given a pair of actions and returns the observations.
        """  

        self.update_left_paddle(action_1)
        self.update_right_paddle(action_2)
        ...
        state_1 = self.check_states(state_1)
        ...
        state_2 = self.check_states(state_2)
        ...
        if self.reward_scheme =="HIT-BASED":
            ...
            reward_1 = 10.0 if self.hit_paddle_left else 0.0
            reward_2 = 10.0 if self.hit_paddle_right else 0.0
            ...
            reward_1 = 1 + -abs(self.ball_y - self.left_y) / self.height
            reward_2 = 1 + -abs(self.ball_y - self.right_y) / self.height
          
        self.number_hits[0] += 1 if self.hit_paddle_left else 0.0
        self.number_hits[1] += 1 if self.hit_paddle_right else 0.0
        

        return state_1, state_2, reward_1, reward_2, self.number_hits, done, {}
        

    def render(self, screen_width=640, screen_height=500, mode="human"):
        """
    ...
    def soft_reset(self):
        """
        This method provides a way for the environment to reset the game state without resetting the environment itself,
        this is useful if goal scoring is enabled.
        """
        # init ball position
    ...
    def reset(self):
        """
        This method completely resets the environment, used at the end of an episode.
        Also resets all the metrics.
        """
        # reset score tally
        self.score_tally = [0, 0]
        self.number_hits = [0, 0]
...
class Rect(object):
    """
    This is a helper class, used by the ball and the paddles to detect collisions.
    """
